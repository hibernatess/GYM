...
function getuser() {
    ...
    $.getJSON("useraction_getUser.action", function (data) {
        data2 = data;
    });
    if (data2 == null) {
        alert("登录失效，请重新登录");
...
setInterval(function () {
    $.ajaxSettings.async = false;
    $.getJSON("useraction_Userpresence.action", function (data) {
        if (!data) {
            alert("登录失效，请重新登录");
            location.href = "login.html";
        }
...
function getmenuTreeList() {
    var data2;
    $.ajaxSettings.async = false;
    $.getJSON("righaction_menuTreelist.action", {role_id: vue.user.roleid, parent_code: -1}, function (data) {
        //返回处理的方法
        layui.tree({
                ...
                if (exist > 0) {
                    ...
                    if (node.attributes.right_url != null && node.attributes.right_url.length > 0) {//判断是否需要新增选项卡;
                        element.tabAdd('tabs', {
                            title: node.name
                            ,
                            content: '<iframe scrolling="yes" frameborder="0"    width="99%" height="80%" src="' + node.attributes.right_url + '" style="color: #009688 "></iframe>'//支持传入html
                            ,
                            id: node.id
                        });
                        element.tabChange('tabs', node.id);//切换到已有的选项卡
